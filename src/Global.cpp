...
// Global JSON document for configuration
JsonDocument doc;
...
void clAssembly::processKeys()
{
        ...
        if (keys[0].edge)
        {
            ...
            state = StateTare;
            OledDisplay.renderQueued("TARA 0", "Apply ", "no force", "", 1000);
        ...
        else if (keys[1].edge)
        {
            ...
            OledDisplay.renderQueued("SCALE", "ApplyForce:", String(cfg.taraCalibrateNewton).c_str(), "", 1000);
            char tftLine2[24];
